...
public class GITLContext {
   ...
   /** The map of encountered labels */
   private HashMap<String, Integer> labelMap = new HashMap<String, Integer>();

   /** The activation stack */
   private Stack<ActivationFrame> activationStack = new Stack<ActivationFrame>();
   /** The map of functions */
   private HashMap<String, FunctionDef> functionMap = new HashMap<String, FunctionDef>();
    ...
    * not find the label in its label map. This means that this label is further
   ...
   public void addLabel(String label) throws GITLException {
       ...
       if ((lineNum = labelMap.get(label)) != null) {
   ...
   public void addVariable(String variable) throws GITLException {
       ...
       if ((def = frame.variableMap.get(variable)) != null) {
           if (def.lineNumber != currentLine) {
                throw new GITLException("Duplicate variable:" + variable);
           }
       } else {
            frame.variableMap.put(variable, new VariableDef(currentLine, 0));
       }
   }
   /**
   ...
   public void renameVariable(String oldName, String newName) throws GITLException {
       ...
       VariableDef def = null;
       if ((def = frame.variableMap.get(oldName)) != null) {
           frame.variableMap.remove(oldName);
           frame.variableMap.put(newName, new VariableDef(currentLine, def.value));
           return;
       }
   ...
   public void setVariableValue(String variable, int value) throws GITLException {
       ...
       if (frame.variableMap.containsKey(variable)) {
           frame.variableMap.put(variable, new VariableDef(currentLine, value));
           return;
       }
   ...
   public void jumpLabel(String label) {
       ...
       Integer lineNum = labelMap.get(label);
   ...
   public void jumpFunction(String functionName) throws GITLException {
       ...
       if ((func = functionMap.get(functionName))!= null) {
            ...
            renameVariable("__callParam", func.paramName);

            currentLine = func.lineNum;
            jumpToFunction = "";
            return;
   ...
   public void addFunction(String functionName, String paramName) throws GITLException {
       FunctionDef func = null;
       if ((func = functionMap.get(functionName))!= null) {
           if (func.lineNum != currentLine) {
               throw new GITLException("Duplicate function definition:" + functionName);
           }
       } else {
           functionMap.put(functionName, new FunctionDef(currentLine, paramName));
       }
   }

    /**
    * Function definition structure. Contains the line number and parameter.
    */
    private static class FunctionDef {
        private int lineNum;
        private String paramName;
        FunctionDef(int ln, String pName) {
            lineNum = ln;
            paramName = pName;
        }
    }
    public void setLastTime(Long ref) {
        lastTime = ref;
    }
   ...
   /**
    * Variable definition structure. Contains the declaration line no and value of
    * the variable.
    */
    private static class VariableDef {
        /** The line at which this variable is defined */
        private int lineNumber;
        /** The value of the variable */
        private int value;
        /**
         * Constructs a VariableDef object
         * @param lineNo The line at which this variable is defined.
         * @param val The value of the variable
         */
        VariableDef(int lineNo, int val) {
            lineNumber = lineNo;
            value = val;
        }
    }
    ...
    private static class ActivationFrame {
        ...
        private HashMap<String, VariableDef> variableMap = new HashMap<String, VariableDef>();
